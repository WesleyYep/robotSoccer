...
public class PenaltyKeeper extends Action {
    ...
    //non-static initialiser block
    {
        ...
        parameters.put("direction", 0); //0 is up, 1 is down
    ...
    public void execute() {
        System.out.println();
        if (System.currentTimeMillis() - GameState.getInstance().getLastStartedTime() < 1000) {
           // System.out.println("under 1 second");
            int direction = parameters.get("direction");
            int goalLine = parameters.get("goalLine");

            if (direction == 0) {
                MoveToSpot.pidMove(bot, goalLine, 80);
            } else {
                MoveToSpot.pidMove(bot, goalLine, 100);
            }
            return;
        }
